...
// Walk a source string, locate every org_id cookie write, and return
// the full call argument list (from the opening paren of .set( through
// the matching closing paren). This lets us inspect the options object
// per call independently.
function extractOrgIdSetCalls(src: string): string[] {
  const results: string[] = [];
  const needles = ['cookieStore.set("org_id"', 'cookies.set("org_id"'];
...
describe("F-0007: org_id cookie must set `secure` flag", () => {
  for (const rel of SITES) {
    it(`every org_id cookie write in ${rel} passes \`secure\``, () => {
      const abs = path.join(ROOT, rel);
      const src = readFileSync(abs, "utf8");
      const calls = extractOrgIdSetCalls(src);
      expect(
        calls.length,
        `expected at least one org_id cookie write in ${rel}`,
      ).toBeGreaterThan(0);
